...
(function(jtree, $, undefined) {
  ...
  jtree.loadData = function(container, data, vupperGap){
   upperGap = vupperGap;
      if (data !== undefined) {
               scene = new THREE.Scene();
  ...
  function onDocumentDBClick(event) {
    ...
    mouse.y = -((event.clientY - upperGap) / (window.innerHeight - upperGap)) * 2 + 1;
    ...
    if (intersects.length > 0) {
      ...
      for (var i = 0; i < intersects.length; i++) {
        if (intersects[i].object.type === "Mesh") {
          console.log("Mesh " + intersects[i].object.geometry.name);
          angular.element(document.getElementById( 'controllerId')).controller().treeAction(intersects[i].object.geometry.name);
        } else {
          console.log("No mesh " + intersects[i].object.type);
        }
      }
    }
  }
  ...
  function addParticle(root, linePos, colors, scene) {
    ...
    var material = new THREE.MeshLambertMaterial({
      color: new THREE.Color("rgb("+root.color.red+","+ root.color.green+","+ root.color.blue+")")
    });
